...
You can choose to ping localhost:port(/path) after Passenger has been restarted, it's done asynchronously so it won't block other guards execution.
If localhost:port(/path) respond with a non-5XX response, Passenger is considered as running, otherwise, there is probably a problem with Passenger.
...
Guard::Passenger accepts some options for configuration.
    ...
    # :ping          boolean or string  ping localhost:port(/ping) after Passenger restart  (default: false)
